...
%%| fig-height: 2.6
graph LR
  A[766<br/>merged] --> B[669<br/>deduped]
  B --> C[58<br/>title-filter]
  C --> D[33<br/>abstract-screen]
  D --> E[6<br/>included]
  classDef step fill:#e9ecef,stroke:#495057,color:#212529;
  classDef warn fill:#fff3cd,stroke:#cc6600,color:#7a4e00;
  classDef final fill:#1f4e79,stroke:#0f2d4a,color:#ffffff;
  class A,B step;
  class C warn;
  class D step;
  class E final;
...
%%| fig-height: 3.6
graph LR
  S[6 studies] --> C[Crude<br/>k=4]
  S --> J[Adjusted<br/>k=2]
  C --> CE[Early mort.<br/>k=1 ❌]
  C --> CL[Long-term<br/>k=2 ⚠️]
  J --> JJ[Mansoor<br/>+ Canet<br/>k=2 ⚠️]
  classDef root fill:#1f4e79,stroke:#0f2d4a,color:#ffffff;
  classDef branch fill:#e9ecef,stroke:#495057,color:#212529;
  classDef leafok fill:#fff3cd,stroke:#856404,color:#5a3e00;
  classDef leafbad fill:#f8d7da,stroke:#721c24,color:#4a1014;
  class S root;
  class C,J branch;
  class CE leafbad;
  class CL,JJ leafok;
...
%%| fig-height: 4.4
graph LR
  Q[TLS in<br/>lymphoma] --> W{When?}
  W -->|Before<br/>chemo| SP[Spontaneous]
  W -->|Days 1–7| TX[Treatment-<br/>induced]
  SP --> HI[High-risk<br/>ICU prephase]
  TX --> O{Organ<br/>dysfunc?}
  O -->|No| WD[Lab TLS<br/>→ Ward]
  O -->|Yes| IC[Clinical TLS<br/>→ ICU + RRT]
  classDef root fill:#1f4e79,stroke:#0f2d4a,color:#ffffff;
  classDef decide fill:#e9ecef,stroke:#495057,color:#212529;
  classDef bad fill:#f8d7da,stroke:#721c24,color:#4a1014;
  classDef ok fill:#d4edda,stroke:#155724,color:#0b2e14;
  classDef neutral fill:#fff3cd,stroke:#856404,color:#5a3e00;
  class Q root;
  class W,O decide;
  class SP,TX neutral;
  class HI,IC bad;
